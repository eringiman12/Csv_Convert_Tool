...
$(function(){
   ...
   $(document).click(function(event) {
    ...
    $("#Select_add").click(function (event) {
      var id_num = 0;
      var sr_ary = [];
      var Max = 0;
      var cnt = 0;
      ...
      $("#Select_Del_Id").children().each(function(index, element){
        id_num = $(this).attr("id").replace("p_","").replace("ple_col_","").replace("_02","");
        sr_ary.push(Number(id_num));
        cnt++;
      })
      
      Max = sr_ary.reduce((a,b)=>Math.max(a,b))
      
      var add_cnt = 0;
      $(".row_0").children().each(function(index, element){
         classname = $(this).attr("class");
         if (classname.indexOf("ple_col_"+Max+"_02_add") !== -1) {
            add_cnt++;
          } 
       })
